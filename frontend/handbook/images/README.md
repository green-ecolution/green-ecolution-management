...
| `tree-form-planned.png`      | 1440×900  | Die Karte (`map`)                                      | The tree form with a planting year set a few years into the future: the hint under the planting year field and the greyed-out, explained sensor select both visible in the same frame. Opened via "Baum bearbeiten" like `tree-form.png`, same panel and window size.                                                                                                                                                        |
...
| `plugin-install.png`         | 1440×900  | Plugins (`settings-plugins`)                           | The plugin install dialog filled in (slug, name, organisation, frontend mode) with both permission matrices visible below it, "Rechte des Plugins" and "Zugriffsrechte für die Ansicht", each with a few permissions checked so the two selections visibly differ from each other. **Needs re-shooting:** the current capture predates the dialog's restyling — "Ansicht" is now a themed dropdown rather than the browser's native select, and the footer buttons sit pinned below a divider instead of scrolling with the form.                       |
