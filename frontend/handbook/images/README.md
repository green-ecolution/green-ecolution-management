...
- `install-ios-safari.png` and `install-android.png` are built from several phone
  ...
  around what the label names, from an edge of the interface rather than through it — in
  `install-android.png` the menu panel is nearly the whole phone screen, cut just below
  the menu's lower edge, and the second panel starts at the top edge of the install sheet
  and keeps it complete down to the app previews.
...
| `install-android.png`        | 1440×1466 | Erste Schritte (`getting-started`)                | Two panels, matching the two numbered steps in the chapter: Chrome's menu open on the app, scrolled so the install entry is in the list, and the install sheet with the **Installieren** button (easiest to trigger on the sensor activation QR screen). Frame the sheet from its top edge down through the app previews; the dimmed page and the address bar above it are not the subject.                                                                                                                                                                                                                                                                                  |
