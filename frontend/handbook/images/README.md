...
| `plugin-install.png`         | 1440×900  | Plugins (`settings-plugins`)                           | The plugin install dialog filled in (slug, name, organisation, frontend mode) with both permission matrices visible below it, "Rechte des Plugins" and "Zugriffsrechte für die Ansicht", each with a few permissions checked so the two selections visibly differ from each other.                                                                                                                                           |
